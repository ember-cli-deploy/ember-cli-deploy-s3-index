...
You can deploy your Ember application to S3 and still use the history-api for pretty URLs. This needs some configuration and the exact process depends on wether or not you are using Cloudfront to serve cached content from your S3 bucket or if you are serving from an S3 bucket directly using S3's Static Website Hosting option. Both options work however the Cloudfront method allows the process to occur without flashing a non-pretty URL in the browser before the application loads.
